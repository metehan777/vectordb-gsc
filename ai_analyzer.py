...
ANALYSIS_PROMPTS = {
    "overview": {
        ...
        "system": (
            "You are an expert SEO analyst. Analyze the Google Search Console data provided. "
...
def _gather_context(analysis_key, custom_query=None):
    ...
    if custom_query:
        ...
        n_results = prompt_config.get("n_results", 30)
    ...
    page_results = vector_store.query_collection(
        pages_col, query_text, n_results=min(n_results, 20)
    )
...
def run_analysis(analysis_key, provider="gemini", custom_query=None):
    if analysis_key == "custom":
        if not custom_query:
            ...
            return None
        system_prompt = (
            "You are an expert SEO analyst with access to Google Search Console data. "
    ...
    provider_label = "[bold blue]Gemini Flash[/bold blue]" if provider == "gemini" else "[bold magenta]Claude Opus[/bold magenta]"
    console.print()
    ...
    with Live(
        ...
        context = _gather_context(analysis_key, custom_query)
        if provider == "claude":
...
def interactive_session():
    ...
    console.print("  [dim]Tip: type a question directly or append[/dim] [bold]--claude[/bold] [dim]to use Claude Opus[/dim]\n")
    ...
    while True:
        ...
        provider = "gemini"
        if choice.lower().endswith(" --claude"):
            provider = "claude"
            choice = choice.replace(" --claude", "").strip()
